...
asmlinkage long cs1550_down(struct cs1550_sem *sem) {
    spin_lock(&sem_lock); /* Lock critical region */
    sem->value--;
    if (sem->value < 0) {
        ...
        printk(KERN_WARNING "cs1550_down syscall: pid=%d exited the critical section.\n",  current->pid);
        
    }
    spin_unlock(&sem_lock); /* Unlock critical region */
...
asmlinkage long cs1550_up(struct cs1550_sem *sem) {
    ...
    sem->value++;
    if (sem->value <= 0) {
        printk(KERN_WARNING "cs1550_up  syscall: pid=%d entered critical secttion\n",  current->pid);
        struct task_struct *next_task = NULL;
        if(sem->head != NULL) {
            ; // Queue is not empty; kfree *head
        }
        printk(KERN_WARNING "cs1550_up  syscall: pid=%d exited critical secttion\n",  current->pid);
    }
